...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    if(ll->size%2 == 0){
        for(count1;count1 < (sizell/2);count1++){
            evenlist = temp->item;
            temp = temp->next;

            removeNode(ll,0);
            insertNode(resultFrontList,count1,evenlist);
        }
        ...
        for(count1;count1 < (sizell/2);count1++){
            ...
            removeNode(ll,0);
            insertNode(resultBackList,count1,oddlist);
        }
        return;
    }
    else{
        ...
        for(count1;count1 <= (sizell/2 - 1);count1++){
            oddlist = temp->item;
            temp = temp->next;
